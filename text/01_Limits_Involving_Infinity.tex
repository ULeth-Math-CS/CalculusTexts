...
%\mnote{.5}{The precise version of Definition \ref{def:limit_of_infinity} analogous to the $\epsilon, \delta$ definition from Section \ref{sec:limit_def} is as follows: 
%Let $I$ be an open interval containing $c$, and let $f$ be a function defined on $I$, except possibly at $c$. 
%\begin{itemize}
%\item The \sword{limit of $f(x)$, as $x$ approaches $c$, is infinity}, denoted by  
%\[
...
\displaystyle \lim_{x\rightarrow c^-} f(x) = \infty,
\]
if we can obtain any arbitrarily large value for $f(x)$ by choosing $x$ sufficiently close to $c$, where $a<x<c$.
...
\displaystyle \lim_{x\rightarrow c^+} f(x) = \infty,
\]
if we can obtain any arbitrarily large value for $f(x)$ by choosing $x$ sufficiently close to $c$, where $c<x<b$.
...
\mfigure{.4}{Observing infinite limit as $x\to 1$ in Example \ref{ex_inflim1}.}{fig:inflim1}{figures/fignolimit2}}
...
\mfigure{.2}{Evaluating $\ds\lim_{x\rightarrow 0}\frac1x$.}{fig:oneoverx}{figures/figoneoverx}
}\pagebreak
...
%We say $\ds\lim_{x\rightarrow\infty} f(x)=L$ if $\epsilon>0$ there exists $M>a$ such that if $x > M$, then $|f(x)-L|<\epsilon$.
...
Figure \ref{fig:hzasy}(c) shows that $f(x) = (\sin x)/x$ has even more interesting behavior than at just $x=0$; as $x$ approaches $\pm\infty$, $f(x)$ approaches 0, but oscillates as it does this.\pagebreak 
