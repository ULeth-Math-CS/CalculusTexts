...
\mnote{.5}{\textbf{Factorial:} The expression $4!$ refers to the number $4\cdot3\cdot2\cdot1 = 24$.
...
{Let $\{a_n\}$ be a sequence and let $L$ be a real number. Given any $\epsilon>0$, if an $m$ can be found such that $\lvert a_n-L\rvert <\epsilon$ for all $n>m$, then we say the \textbf{limit of $\{a_n\}$, as $n$ approaches infinity, is $L$}, denoted 
\[
...
This definition is reminiscent of the $\epsilon$--$\delta$ proofs of Chapter \ref{I-chapter:limits}. In that chapter we developed other tools to evaluate limits apart from the formal definition; we do so here as well.
%\clearpage
...
Theorem \ref{thm:seq_limit} allows us, in certain cases, to apply the tools developed in Chapter \ref{I-chapter:limits} to limits of sequences. Note two things \textit{not} stated by the theorem:
	\begin{enumerate}
...
{Let $\{a_n\}$ be a sequence. If $\ds \lim_{n\to\infty} \lvert a_n\rvert = 0$, then $\ds \lim_{n\to\infty} a_n = 0$\index{Absolute Value Theorem}\index{limit!Absolute Value Theorem}\index{sequence!Absolute Value Theorem}
}
...
{\begin{enumerate}
\item		This appeared in Example \ref{ex_seq4}. We want to apply Theorem \ref{thm:abs_val_seq}, so consider the limit of $\{\lvert a_n\rvert\}$:
\begin{align*}
\lim_{n\to\infty} \lvert a_n\rvert &= \lim_{n\to\infty} \left\lvert\frac{(-1)^n}{n}\right\rvert \\
...
\begin{align*}
\lim_{n\to\infty} \lvert a_n\rvert &= \lim_{n\to\infty} \left\lvert\frac{(-1)^n(n+1)}{n}\right\rvert \\
...
\mfigure{.25}{A plot of a sequence in Example \ref{ex_seq5}, part 2.}{fig:seq5}{figures/figseq5}
Since we know that the signs of the terms alternate \emph{and} we know that the limit of $\lvert a_n\rvert$ is 1, we know that as $n$ approaches infinity, the terms will alternate between values close to 1 and $-1$, meaning the sequence diverges. A plot of this sequence is given in Figure \ref{fig:seq5}.
\end{enumerate}
\vskip-\baselineskip
}\\
...
\mtable{.6}{A plot of $\{a_n\} = \{1/n\}$ and $\{a_n\} = \{2^n\}$ from Example \ref{ex_seq3}.}{fig:seq3}{
\begin{tabular}{c}
...
\mnote{.23}{\textbf{Note:} Keep in mind what Theorem \ref{thm:converge_bounded} does \emph{not} say. It does not say that bounded sequences must converge, nor does it say that if a sequence does not converge, it is not bounded.}

In Example \ref{ex_seq6} we saw the sequence $\ds \{b_n\} = \left\{\left(1+1/n\right)^{n}\right\}$, where it was stated that $\ds \lim_{n\to\infty} b_n = e$. (Note that this is simply restating part of Theorem \ref{I-thm:special_limits}.) Even though it may be difficult to intuitively grasp the behaviour of this sequence, we know immediately that it is bounded.
...
\mfigure{.4}{A plot of $\{a_n\} = \{(n+1)/n\}$ in Example \ref{ex_seq7}.}{fig:seq7a}{figures/figseq7a}
...
Since $a_{n+1}-a_n>0$ for all $n$, we conclude the sequence is increasing; see Figure \ref{fig:seq7b}(a).
%\drawexampleline
%\mfigure{.5}{A plot of $\{a_n\} = \{n^2/n!\}$ in Example \ref{ex_seq7}.}{fig:seq7d}{figures/figseq7d}

\item		We can clearly see in Figure \ref{fig:seq7b}(b), where the sequence is plotted, that it is not monotonic. However, it does seem that after the first 4 terms it is decreasing. To understand why, perform the same analysis as done before:
...
\mtable{.55}{Plots of sequences in Example \ref{ex_seq7}.}{fig:seq7b}{%
\begin{tabular}{c}
\myincludegraphics{figures/figseq7b}\\
(a)\rule[-25pt]{0pt}{10pt}\\ 
\myincludegraphics{figures/figseq7c}\\
(b)\rule[-25pt]{0pt}{10pt}\\ 
\myincludegraphics{figures/figseq7d}\\
(c)\\
\end{tabular}
}
...
\item		Again, the plot in Figure \ref{fig:seq7b}(c) shows that the sequence is not monotonic, but it suggests that it is monotonically decreasing after the first term. We perform the usual analysis to confirm this.
