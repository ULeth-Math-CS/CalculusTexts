...
Recall that many limits that result in indeterminate forms can be handled using \sword{l'Hospital's Rule}\index{l'Hospital's Rule}. We briefly recall the statement of the theorem: suppose that functions $f$ and $g$ are differentiable on an open interval containing $a$, and that either 
\[
...
\mnote{.2}{Note that l'Hosptial's rule can also be applied in the case of limits where $x\to \pm\infty$.}

\pagebreak

\example{ex_impint2}{Improper integration and l'Hospital's Rule}{
...
\int_1^\infty\frac{\ln x}{x^2}\ dx &= \lim_{b\to\infty}\int_1^b\frac{\ln x}{x^2}\ dx \\
			...
			\intertext{The $1/b$ and $\ln 1$ terms go to 0, leaving $\ds \lim_{b\to\infty} -\frac{\ln b}b + 1.$ We need to evaluate $\ds \lim_{b\to\infty} \frac{\ln b}{b}$ with l'Hospital's Rule. We have:}
...
Our first tool is to understand the behaviour of functions of the form $\ds \frac1{x\hskip1pt ^p}$.\\
...
\mnote{.1}{\textbf{Note:} We used the upper and lower bound of ``1'' in Key Idea \ref{idea:impint1} for convenience. It can be replaced by any $a$ where $a>0$. 
}
...
{As $x$ gets large, the denominator of the integrand will begin to behave much like $y=x$. So we compare \small$\ds\frac{1}{\sqrt{x^2+2x+5}}$\normalsize\ to \small$\ds\frac1x$\normalsize\ with the Limit Comparison Test:
...
\]
This converges to 1, meaning the original limit also converged to 1. As $x$ gets very large, the function 
